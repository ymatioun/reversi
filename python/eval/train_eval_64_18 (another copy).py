...
ver = 0 # global verbose switch ************************

for lc1 in range(5): # 5
    ...
    with tf.device('/GPU:0'):
        ...
        d1 = 1 + Embedding(input_dim=x[:,0].max()+1, output_dim=E1, mask_zero=False, name='Nm', embeddings_initializer=tf.keras.initializers.Zeros(), embeddings_regularizer=tf.keras.regularizers.L2(4*L2))(i1[:,0]) # 1xE1
        #d1 = Tr23()(d1) # E1x1
        d1 = Resh()(d1) # E1x1
        ...
        d3 = Concatenate()([d3c, d3d, d3e, d3f, d3g, d3h, d3i, d3j, d3k, d3l, d3a, d3b, d3c8, d3m]) # E1x14
        
        d4a = MyLayer()(d3) # E1, relu
        o = MySum2()(d4a) # +-
        ...
        model.fit(x=tr, epochs=26, validation_data=va, verbose=2*ver, callbacks=[es]) # print_lr()
    ...
    print(E1, s,  int(time()-t0), 'sec')
# add N1         26.0 - no improvement?
# add dN         25.9
# mult dN        26.1
...
# 32 27.3 488 sec  -2.3
# 64 26.0 750 sec  -1.3
#128 25.4 1138 sec -0.6
#256 25.2 1782 sec -0.2
