...
#	char sc;			//1, true score
#	unsigned char tmp;	        //1, placeholder
#} posit;
...
with open('../temp/pos_bin2.bin', 'rb') as file: # 409,209,060 = 20,460,453 positions
    i = 0
    x = YourStruct()
...
# shuffle the data
np.random.seed(13)
idx = np.arange(posb.shape[0])
np.random.shuffle(idx)
posb = posb[idx,:]
mob  = mob[idx,:]
sc   = sc[idx]
print('shuffled train data',int(time()-t0), 'sec')
