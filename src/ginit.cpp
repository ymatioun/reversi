...
static void init_hash(void), init_arrays(void), init_eval(void), init_sigmas(void);
...
void Ginit(void){
	...
	FILE *f1 = fopen("//home//youri//reversi//temp//pos_bin0.bin", "rb");	// input file
    ...
    f1=fopen("//home//youri//reversi//temp//pos_eval.csv", "w"); // output file
	fprintf(f1, "n,sc,ev\n");
	...
	// save new file
	f1 = fopen("//home//youri//reversi//temp//pos_bin1.bin", "wb");
	...
	// save new file
	f1 = fopen("//home//youri//reversi//temp//pos_bin2.bin", "wb");
	fwrite(p0, sizeof(posit), pos_count, f1);
	fclose(f1); exit(0);*/
    ...
    /*f1=fopen("//home//youri//reversi//temp//pos_sigma.csv", "w"); // output file
	...
	f1=fopen("//home//youri//reversi//temp//sigmas_even_2.csv", "w"); // output file *********************************
	TimePoint t1, t2;
...
static void init_hash(){
    // allocate main TT
    UINT64 sz = HASH_SIZE + 10;
	sz *= sizeof(hashtype);
	h = (hashtype*)aligned_alloc(2 * 1024 * 1024, sz); // large memory pages (2 Mb)
	madvise((void*)h, sz, MADV_HUGEPAGE);

	// second version
	h2 = (hashtype*)aligned_alloc(2 * 1024 * 1024, sz); // large memory pages (2 Mb)
	madvise((void*)h2, sz, MADV_HUGEPAGE);

	clear_hash(); // this also inits history
}

static void init_arrays(){// allocate params and other vars
    const unsigned int sizes[] = {10 * 1024 * sizeof(unsigned int) / sizeof(eval_type) / EVAL_WIDTH, 3 * 512 * sizeof(unsigned int) / sizeof(eval_type) / EVAL_WIDTH, 1024 * sizeof(unsigned int) / sizeof(eval_type) / EVAL_WIDTH, 24, 24, 81*81, 81*81, 81*81, 81*81, 81*81, 81*27, 81*9, 81*3, 81, 81*81*9, 81*81*3, 81*81*9};
    unsigned int st = 0, i;
	...
	t += 1024 * 10 * sizeof(unsigned int) / sizeof(eval_type); // 10 patterns of up to 10 bits;
	...
	for(unsigned int i10=0; i10<2; ++i10){
        ...
        // need to translate from 3 c10a patterns (0 1 2 5 6 7 9 10 13 14)
        tr_ar_c10[1024 * 7 + ii] = i1 + i3 * 2 + i5 * 4 + i7 * 8 + i9 * 16 + i10 * 32 + i2 * 64 + i4 * 128 + i6 * 256 + i8 * 512; // T to R
        tr_ar_c10[1024 * 8 + ii] = i1 + i2 * 2 + i4 * 4 + i6 * 8 + i8 * 16 + i10 * 32 + i3 * 64 + i5 * 128 + i7 * 256 + i9 * 512; // T to L
        tr_ar_c10[1024 * 9 + ii] = i5 + i6 * 2 + i7 * 4 + i8 * 8 + i9 * 16 + i10 * 32 + i1 * 64 + i2 * 128 + i3 * 256 + i4 * 512; // T to B
    }
}
...
static void init_eval(){ // load eval coeffs (v15)
    // l_s8a - 3005 integer values
    unsigned int l_s8a_s = 0;
    FILE *f = fopen("//home//youri//reversi//train//eval_v18//l_s8a.txt", "r");
    int l_s8a[6561];
    while(1){
        if( fscanf(f, "%d\n", l_s8a + l_s8a_s) < 1 )
            break;
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//l_s8b.txt", "r");
    int l_s8b[6561];
    while(1){
        if( fscanf(f, "%d\n", l_s8b + l_s8b_s) < 1 )
            break;
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//l_s8c.txt", "r");
    int l_s8c[6561];
    while(1){
        if( fscanf(f, "%d\n", l_s8c + l_s8c_s) < 1 )
            break;
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//l_s8d.txt", "r");
    int l_s8d[6561];
    while(1){
        if( fscanf(f, "%d\n", l_s8d + l_s8d_s) < 1 )
            break;
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//l_d8.txt", "r");
    int l_d8[6561];
    while(1){
        if( fscanf(f, "%d\n", l_d8 + l_d8_s) < 1 )
            break;
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//l_d7.txt", "r");
    int l_d7[2187];
    while(1){
        if( fscanf(f, "%d\n", l_d7 + l_d7_s) < 1 )
            break;
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//l_d6.txt", "r");
    int l_d6[729];
    while(1){
        if( fscanf(f, "%d\n", l_d6 + l_d6_s) < 1 )
            break;
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//l_d5.txt", "r");
    int l_d5[243];
    while(1){
        if( fscanf(f, "%d\n", l_d5 + l_d5_s) < 1 )
            break;
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//l_d4.txt", "r");
    int l_d4[81];
    while(1){
        if( fscanf(f, "%d\n", l_d4 + l_d4_s) < 1 )
            break;
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//l_c8.txt", "r");
    int l_c8[33000];
     while(1){
        if( fscanf(f, "%d\n", l_c8 + l_c8_s) < 1 )
            break;
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//l_c9.txt", "r");
    int l_c9[5000];
     while(1){
        if( fscanf(f, "%d\n", l_c9 + l_c9_s) < 1 )
            break;
    ...
    // l_c10a - 20000 integer values
    unsigned int l_c10a_s = 0;
    f = fopen("//home//youri//reversi//train//eval_v18//l_c10a.txt", "r");
    int l_c10a[20000];
     while(1){
        if( fscanf(f, "%d\n", l_c10a + l_c10a_s) < 1 )
            break;
    ...
    assert(l_c10a_s < sizeof(l_c10a) / sizeof(l_c10a[0])); // make sure there is no overflow on read
    fclose(f);

    // c_s8a - x EVAL_WIDTH short int values ******************************************************************
    f = fopen("//home//youri//reversi//train//eval_v18//c_s8a.txt", "r");
    short int *c_s8a = (short int *)malloc(6561 * EVAL_WIDTH*sizeof(short int));
    for(unsigned int i=0; i<l_s8a_s * EVAL_WIDTH; ++i)
        fscanf(f, "%hi\n", c_s8a + i);
    fclose(f);

    // c_s8b - x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//train//eval_v18//c_s8b.txt", "r");
    short int *c_s8b = (short int *)malloc(6561 * EVAL_WIDTH*sizeof(short int));
    for(unsigned int i=0; i<l_s8b_s * EVAL_WIDTH; ++i)
        fscanf(f, "%hi\n", c_s8b + i);
    fclose(f);

    // c_s8c - x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//train//eval_v18//c_s8c.txt", "r");
    short int *c_s8c = (short int *)malloc(6561 * EVAL_WIDTH*sizeof(short int));
    for(unsigned int i=0; i<l_s8c_s * EVAL_WIDTH; ++i)
        fscanf(f, "%hi\n", c_s8c + i);
    fclose(f);

    // c_s8d - x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//train//eval_v18//c_s8d.txt", "r");
    short int *c_s8d = (short int *)malloc(6561 * EVAL_WIDTH*sizeof(short int));
    for(unsigned int i=0; i<l_s8d_s * EVAL_WIDTH; ++i)
        fscanf(f, "%hi\n", c_s8d + i);
    fclose(f);

    // c_d8 - x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//train//eval_v18//c_d8.txt", "r");
    short int *c_d8 = (short int *)malloc(6561 * EVAL_WIDTH*sizeof(short int));
    for(unsigned int i=0; i<l_d8_s * EVAL_WIDTH; ++i)
        fscanf(f, "%hi\n", c_d8 + i);
    fclose(f);

    // c_d7 - x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//train//eval_v18//c_d7.txt", "r");
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//c_d6.txt", "r");
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//c_d5.txt", "r");
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//c_d4.txt", "r");
    ...
    f = fopen("//home//youri//reversi//train//eval_v18//c_c8.txt", "r");
    short int *c_c8 = (short int *)malloc(sizeof(l_c8) / sizeof(l_c8[0]) * EVAL_WIDTH*sizeof(short int));
    for(unsigned int i=0; i<l_c8_s * EVAL_WIDTH; ++i)
        fscanf(f, "%hi\n", c_c8 + i);
    fclose(f);

    // c_c9 - x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//train//eval_v18//c_c9.txt", "r");
    short int *c_c9 = (short int *)malloc(sizeof(l_c9) / sizeof(l_c9[0]) * EVAL_WIDTH*sizeof(short int));
    for(unsigned int i=0; i<l_c9_s * EVAL_WIDTH; ++i)
        fscanf(f, "%hi\n", c_c9 + i);
    fclose(f);

    // c_c10a - x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//train//eval_v18//c_c10a.txt", "r");
    short int *c_c10a = (short int *)malloc(sizeof(l_c10a) / sizeof(l_c10a[0]) * EVAL_WIDTH*sizeof(short int));
    for(unsigned int i=0; i<l_c10a_s * EVAL_WIDTH; ++i)
        fscanf(f, "%hi\n", c_c10a + i);
    fclose(f);

    // mob1 - 24 x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//train//eval_v18//c_mob1.txt", "r");
    for(unsigned int i=0; i<24 * EVAL_WIDTH; ++i)
        fscanf(f, "%hi\n", &mob1_c[i]);
    fclose(f);

    // mob2 - 24 x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//train//eval_v18//c_mob2.txt", "r");
    for(unsigned int i=0; i<24 * EVAL_WIDTH; ++i)
        fscanf(f, "%hi\n", &mob2_c[i]);
    fclose(f);

    // c_n - 49 x EVAL_WIDTH float values
    f = fopen("//home//youri//reversi//train//eval_v18//c_n.txt", "r");
    for(unsigned int i=0; i<49; ++i){
        unsigned int j;
        for(j=0; j<EVAL_WIDTH-1; ++j)
            fscanf(f, "%f ", &n_c[i][j]);
        fscanf(f, "%f\n", &n_c[i][j]);
        for(j=0; j<EVAL_WIDTH; ++j)
            if(n_c[i][j] > 0)
                n_ci[i][j] = (eval_type)((n_c[i][j] + 0.49f) * 4); // populate int coeffs, with a multiple
            else
                n_ci[i][j] = (eval_type)((n_c[i][j] - 0.49f) * 4); // populate int coeffs, with a multiple
    }
    fclose(f);
    ...
    // now load reduced c10a (0 1 2 5 6 7 9 10 13 14) coeffs into full coeff array
    for(int i1=-1; i1<2; ++i1)
    for(int i2=-1; i2<2; ++i2)
    for(int i3=-1; i3<2; ++i3)
    for(int i4=-1; i4<2; ++i4)
	...
	for(int i10=-1; i10<2; ++i10){
        j1 = i1 + i2 * 3 + i3 * 9 + i4 * 27 + i5 * 81 + i6 * 81 * 3 + i7 * 81 * 9 + i8 * 81 * 27 + i9 * 81 * 81 + i10 * 81 * 81 * 3;
	    j2 = i6 + i5 * 3 + i4 * 9 + i3 * 27 + i2 * 81 + i1 * 81 * 3 + i10 * 81 * 9 + i9 * 81 * 27 + i8 * 81 * 81 + i7 * 81 * 81 * 3;; // symmetry
	    j12 = std::min(j1, j2);

	    int* item = (int*)bsearch(&j12, l_c10a, l_c10a_s, sizeof(int), compare);
	    if (item != NULL){
            k = item - l_c10a; // index of found key
            for(unsigned int l=0; l<EVAL_WIDTH; ++l){// copy coeffs into both symmetries
                c10a_c[(29524 + j1) * EVAL_WIDTH + l] = c_c10a[k * EVAL_WIDTH + l];
                c10a_c[(29524 + j2) * EVAL_WIDTH + l] = c_c10a[k * EVAL_WIDTH + l];
            }
        }
    }
    ...
    free(c_s8a);free(c_s8b);free(c_s8c);free(c_s8d);free(c_d8);free(c_c8);free(c_c9);free(c_c10a);
}

static void init_sigmas(){// load sigma coeffs
    // l_s8a - 3005 integer values
    unsigned int l_s8a_s = 0;
    FILE *f = fopen("//home//youri//reversi//temp//sigma_v3//l_s8a.txt", "r");
    int l_s8a[6561];
    while(1){
        if( fscanf(f, "%d\n", l_s8a + l_s8a_s) < 1 )
            break;
    ...
    unsigned int l_s8b_s = 0;
    f = fopen("//home//youri//reversi//temp//sigma_v3//l_s8b.txt", "r");
    int l_s8b[6561];
    while(1){
        if( fscanf(f, "%d\n", l_s8b + l_s8b_s) < 1 )
            break;
    ...
    unsigned int l_s8c_s = 0;
    f = fopen("//home//youri//reversi//temp//sigma_v3//l_s8c.txt", "r");
    int l_s8c[6561];
    while(1){
        if( fscanf(f, "%d\n", l_s8c + l_s8c_s) < 1 )
            break;
    ...
    unsigned int l_s8d_s = 0;
    f = fopen("//home//youri//reversi//temp//sigma_v3//l_s8d.txt", "r");
    int l_s8d[6561];
    while(1){
        if( fscanf(f, "%d\n", l_s8d + l_s8d_s) < 1 )
            break;
    ...
    // l_c9 - 5154 integer values
    unsigned int l_c9_s = 0;
    f = fopen("//home//youri//reversi//temp//sigma_v3//l_c9.txt", "r");
    int l_c9[5000];
     while(1){
        if( fscanf(f, "%d\n", l_c9 + l_c9_s) < 1 )
            break;
    ...
    f = fopen("//home//youri//reversi//temp//sigma_v3//l_c10.txt", "r");
    int l_c10[33000];
     while(1){
        if( fscanf(f, "%d\n", l_c10 + l_c10_s) < 1 )
            break;
    ...
    // c_s8a - x EVAL_WIDTH short int values ******************************************************************
    f = fopen("//home//youri//reversi//temp//sigma_v3//c_s8a.txt", "r");
    ...
    // c_s8b - x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//temp//sigma_v3//c_s8b.txt", "r");
    ...
    // c_s8c - x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//temp//sigma_v3//c_s8c.txt", "r");
    ...
    // c_s8d - x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//temp//sigma_v3//c_s8d.txt", "r");
    ...
    // c_c9 - x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//temp//sigma_v3//c_c9.txt", "r");
    ...
    f = fopen("//home//youri//reversi//temp//sigma_v3//c_c10.txt", "r");
    ...
    f = fopen("//home//youri//reversi//temp//sigma_v3//c_mob1.txt", "r");
    for(unsigned int i=0; i<22 * 16; ++i)
        fscanf(f, "%hi\n", &mob1_c2[0][i]);
    fclose(f);

    // mob2 - 22 x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//temp//sigma_v3//c_mob2.txt", "r");
    for(unsigned int i=0; i<22 * 16; ++i)
        fscanf(f, "%hi\n", &mob2_c2[0][i]);
    fclose(f);

    // c_n - 41 x EVAL_WIDTH short int values
    f = fopen("//home//youri//reversi//temp//sigma_v3//c_n.txt", "r");
    for(unsigned int i=0; i<41 * 16; ++i)
        fscanf(f, "%hi\n", &n_c2[0][i]);
    fclose(f);

    // d_c2 - 16 x 18 float values
    f = fopen("//home//youri//reversi//temp//sigma_v3//c_dd.txt", "r");
