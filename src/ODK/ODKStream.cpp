...
void CODKStream::TellGGS(const char* pmsg) {
	(*this) << pmsg;
	flush();
...
// pass a message to GGS
void CODKStream::TellGGS0(const char* pmsg) {
	(*this) << pmsg;
	flush();
}
...
void CODKStream::HandleGGSTell(const CMsgGGSTell* pmsg) {
	...
	if (pmsg->sFrom=="ymatiou2" || pmsg->sFrom=="ymatiou4") { // defines "master" - commands from master are just executed
		if (pmsg->sText=="quit"){
			Logout();
			exit(0);
		}
