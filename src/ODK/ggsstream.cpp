...
const string& ggsstream::GetLogin() const {return sLogin;}
...
CMsg* ggsstream::GetMsgType(istream& is) {
	...
	if (sFrom.empty())
		pmsg=NULL;
	else {// direct messages end in ':', channel messages don't
		if (sFrom.end()[-1]==':')
...
CMsg* ggsstream::GetMsgTypeGGS(istream& is) {
	...
	if (sMsgType=="alias")
		...
		pmsg=new CMsgGGSWho;
	else if (sMsgType=="+" || sMsgType=="-") {// begin/end game
		bool fPlus= sMsgType=="+";
...
void ggsstream::BaseOsMatch(const CMsgOsMatch* pmsg) {
	idToMatch.clear();
	//const COsMatch* i; // problem!
	std::vector<COsMatch>::const_iterator i;
	i=pmsg->matches.begin();
	for (;i!=pmsg->matches.end();i++)
		idToMatch[i->idm]=*i;
}
...
void ggsstream::HandleOsRank(const CMsgOsRank* pmsg) {HandleOs(pmsg);}

void ggsstream::HandleOsRatingUpdate(const CMsgOsRatingUpdate* pmsg) {HandleOs(pmsg);}

void ggsstream::HandleOsRequestDelta(const CMsgOsRequestDelta* pmsg) {BaseOsRequestDelta(pmsg);HandleOs(pmsg);}

void ggsstream::HandleOsStored(const CMsgOsStored* pmsg) {HandleOs(pmsg);}

void ggsstream::HandleOsTimeout(const CMsgOsTimeout* pmsg) {HandleOs(pmsg);}

void ggsstream::HandleOsTop(const CMsgOsTop* pmsg) {HandleOs(pmsg);}

void ggsstream::HandleOsTrustViolation(const CMsgOsTrustViolation* pmsg) {HandleOs(pmsg);}

void ggsstream::HandleOsUndoRequest(const CMsgOsUndoRequest* pmsg) {HandleOs(pmsg);}

void ggsstream::HandleOsUnknown(const CMsgOsUnknown* pmsg) {HandleOs(pmsg);}
...
void ggsstream::HandleOsWatch(const CMsgOsWatch* pmsg) {HandleOs(pmsg);}

void ggsstream::HandleOsWatchDelta(const CMsgOsWatchDelta* pmsg) {HandleOs(pmsg);}
