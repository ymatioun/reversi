...
void COsMoveListItem::Out(ostream& os) const {
	int nPrecisionOld=(int)os.precision(2);
...
void COsGame::Out(ostream& os) const {
	...
	//const COsMoveListItem* pmli;// problem!
	std::vector<COsMoveListItem>::const_iterator pmli;
	pmli=ml.begin();
	for (; pmli!=ml.end(); pmli++){
...
void COsGame::CalcCurrentPos() {
	...
	//COsMoveListItem* pmli; // problem!
	std::vector<COsMoveListItem>::iterator pmli;
	pmli=ml.begin();
	for (; pmli!=ml.end(); pmli++)
		pos.Update(*pmli);
}
