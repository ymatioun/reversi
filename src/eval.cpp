...
# define S 2 // number of mm256 vars in use
int eval(board *b, UINT64 move_mask){
    __m256i w1[S], w2[S];
    unsigned int v1, v2, i, j;

    // mob1 *****************************************************************************************
    unsigned int mob1 = std::min(23,(int)_popcnt64(move_mask)); // my mob, capped at 23
    for(j=0; j<S; ++j)
        w1[j] = _mm256_load_si256((__m256i*)&mob1_c[mob1 * EVAL_WIDTH + 16 * j]);

    // mob2 *****************************************************************************************
    unsigned int mob2 = std::min(23,(int)_popcnt64(bit_mob_mask(b->pos[1], b->pos[0]))); // opp mob, capped at 23
    for(j=0; j<S; ++j){
    ...
    // a. save epi16
    int16_t t1[32];
    _mm256_store_si256((__m256i*)t1, w1[0]);
    _mm256_store_si256((__m256i*)(t1+16), w1[1]);
    // b. load as epi32
    __m128i w3a[4];
    __m256i w3[4];
    w3a[0] = _mm_load_si128((__m128i*)&t1[0]);
    w3a[1] = _mm_load_si128((__m128i*)&t1[8]);
    w3a[2] = _mm_load_si128((__m128i*)&t1[16]);
    w3a[3] = _mm_load_si128((__m128i*)&t1[24]);
    // c. convert 128 to 256
    w3[0] = _mm256_cvtepi16_epi32(w3a[0]);
    w3[1] = _mm256_cvtepi16_epi32(w3a[1]);
    w3[2] = _mm256_cvtepi16_epi32(w3a[2]);
    w3[3] = _mm256_cvtepi16_epi32(w3a[3]);

    // epi32 into ps
    __m256 w4[4];
    w4[0] = _mm256_cvtepi32_ps(w3[0]);
    w4[1] = _mm256_cvtepi32_ps(w3[1]);
    w4[2] = _mm256_cvtepi32_ps(w3[2]);
    w4[3] = _mm256_cvtepi32_ps(w3[3]);

    // load ps multiples
    unsigned int n = _popcnt64(b->pos[0] | b->pos[1]);
    //cc[n] ++;
    n = std::min(60, std::max((int)n, 12)) - 12; // now 0 to 48
    __m256 w5[4];
    w5[0] = _mm256_load_ps(&n_c[n][0]);
    w5[1] = _mm256_load_ps(&n_c[n][8]);
    w5[2] = _mm256_load_ps(&n_c[n][16]);
    w5[3] = _mm256_load_ps(&n_c[n][24]);

    // multiply
    w5[0] = _mm256_mul_ps(w4[0], w5[0]);
    w5[1] = _mm256_mul_ps(w4[1], w5[1]);
    w5[2] = _mm256_mul_ps(w4[2], w5[2]);
    w5[3] = _mm256_mul_ps(w4[3], w5[3]);

    // floor at 0
    __m256 z = _mm256_set1_ps(0.0f);
    w5[0] = _mm256_max_ps(w5[0], z);
    w5[1] = _mm256_max_ps(w5[1], z);
    w5[2] = _mm256_max_ps(w5[2], z);
    w5[3] = _mm256_max_ps(w5[3], z);

    // +- into a single ps
    w5[0] = _mm256_add_ps(w5[0], w5[1]);
    w5[2] = _mm256_add_ps(w5[2], w5[3]);
    w5[0] = _mm256_sub_ps(w5[0], w5[2]);
